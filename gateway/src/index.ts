...
import {
  initPostgres,
  closePostgres
} from "./db/postgres/postgres.client";
...
async function start(): Promise<void> {
  ...
  if (useSQLite) {
    ...
    console.log("[conduit] Using SQLite fallback (config.data.sqlite_fallback = true)");
    try {
      initSQLite();
  ...
  if (hasLicense) {
    try {
      ...
      console.warn("[conduit] Initial license check failed (non-fatal):", err);
    }

    // ── 8. License check loop ─────────────────────────────────────────────
    startLicenseCheckLoop();
  } else {
    console.log("[conduit] License system not configured — running unrestricted");
  }
